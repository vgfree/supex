...
ssize_t cache_vappendf(struct cache *cache, const char *fmt, va_list ap)
{
	...
	if (unlikely(size == CACHE_FMTSIZE)) {
		char t[2] = { };
		va_copy(apcopy, ap);
		size = vsnprintf(t, sizeof(t), fmt, apcopy);
		va_end(apcopy);
		return_val_if_fail(size > 0, -1);
	}
...
ssize_t cache_vinsertf(struct cache *cache, const char *fmt, va_list ap)
{
	...
	va_end(apcopy);
	return_val_if_fail(size > 0, -1);
	/*have enough space at front of start*/
	if (cache->start >= size) {
		char ch = cache->buff[cache->start];
		va_copy(apcopy, ap);
		vsnprintf(&cache->buff[cache->start - size], size + 1, fmt, apcopy);
		va_end(apcopy);
		cache->buff[cache->start] = ch;
		cache->start -= size;
	} else {
		//cache_movemem(cache);
		len = cache_incrmem(cache, size + 1);
		...
		va_copy(apcopy, ap);
		vsnprintf(&cache->buff[cache->start], size + 1, fmt, apcopy);
		va_end(apcopy);
