...
static void init_ctx(int argc, char **argv)
{
	...
	g_iohandle.zmq.skt = zmq_socket(g_iohandle.zmq.ctx, ZMQ_PUSH);
...
static void bind_push()
{
	...
	set_zmqopt(g_iohandle.zmq.skt, ZMQ_SNDHWM, 10);
#ifdef IMMEDIATE
...
static void conn_push()
{
	...
	/*发送缓冲为最小1000*/
	set_zmqopt(g_iohandle.zmq.skt, ZMQ_SNDHWM, 1000);
#ifdef IMMEDIATE
	set_zmqopt(g_iohandle.zmq.skt, ZMQ_IMMEDIATE, 1);
#endif
...
static void start_event()
{
	...
	ev_io_init(&g_eio, io_cb, g_iohandle.zmq.fd, EV_WRITE);
...
static void io_cb(struct ev_loop *loop, struct ev_io *io, int event)
{
	...
	//	flag = check_zmqwrite(g_iohandle.zmq.skt, 1000);
	//	if (unlikely(!flag)) {
	//		x_printf(W, "Not writeable.");
	//	} else {
	int     i = (int)RandInt(4, 10);
	int     j = (int)RandInt(PKG_SIZE / 2, PKG_SIZE);
	int     s = j / i;

	/*模拟装载数据*/
	g_iocache->frames = i;
	g_iocache->offset = offsetof(struct zmqframe, data);

	for (i = 0; i < g_iocache->frames; i++) {
		g_iocache->frame[i] = s;
		g_iocache->offset += s;
	}
	...
	int frames = write_zmqdata(g_iohandle.zmq.skt, g_iocache);

	if (likely(frames > 0)) {
		x_printf(I, "Write data : `%d` : `%d`", frames, s);
