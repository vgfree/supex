...
void freeclient(struct client *client)
{
	...
	if (client->fd > -1) {
		printf("\x1B[1;33m" "`%ld` - connect disconnect: [%d] - [%.*s : %d]" "\x1B[m" "\n",
...
{
	printf("\x1B[1;31m" "`%ld` - one task destroy ..." "\x1B[m" "\n", g_tid);
...
void work(struct evcoro_scheduler *scheduler, void *arg)
{
	...
	/*判断是否连接成功*/
	printf("\x1B[1;33m" "`%ld` - start connect : [%d]" "\x1B[m" "\n", g_tid, client->fd);
test_conn:
	...
	if (likely(rc < 0)) {
		...
		if (likely(code == EISCONN)) {
			printf("\x1B[1;33m" "`%ld` - connection standy : [%d]" "\x1B[m" "\n", g_tid, client->fd);
		} else if (likely((code == EALREADY) || (code == EINPROGRESS))) {
			bool flag = false;
			printf("\x1B[1;33m" "`%ld` - connecting ... : [%d]" "\x1B[m" "\n", g_tid, client->fd);
			/*没有执行协程则休眠一会*/
			...
			if (unlikely(!flag)) {
				/*超时，未就绪*/
				printf("\x1B[1;33m" "`%ld` - connect timed out: [%d]" "\x1B[m" "\n", g_tid, client->fd);
			} else {
				/*可写，测试时候已连接*/
				goto test_conn;
			}
			...
			/*出错*/
			fprintf(stderr, "\x1B[1;31m" "`%ld` - connect () error : [%d] %s" "\x1B[m" "\n", g_tid, client->fd, strerror(code));
			evcoro_stop(scheduler);
			goto clean;
		}
	} else {
		printf("\x1B[1;32m" "`%ld` - new connection : [%d]" "\x1B[m" "\n", g_tid, client->fd);
	}
		...
		if (unlikely(bytes < 0)) {
			...
			if (likely(code == EAGAIN)) {
				...
				if (unlikely(!flag)) {
					...
					/*超时，未就绪*/
					printf("\x1B[1;33m" "`%ld` - read timed out: [%d] - %03lld.%06lld" "\x1B[m" "\n", g_tid, client->fd, ss, us);
					goto clean;
				} else {
					gettimeofday(&tv2, NULL);
					...
					printf("\x1B[1;32m" "`%ld` - read standby: [%d] - %03lld.%06lld" "\x1B[m" "\n", g_tid, client->fd, ss, us);
					continue;
				}
			} else {
				/*出错*/
				fprintf(stderr, "\x1B[1;31m" "`%ld` - read () error : [%d] %s" "\x1B[m" "\n", g_tid, client->fd, strerror(code));
				goto clean;
			}
		} else if (unlikely(bytes == 0)) {
			printf("\x1B[1;33m" "`%ld` - terminal close : [%d]" "\x1B[m" "\n", g_tid, client->fd);
			goto clean;
		} else {
			printf("\x1B[1;32m" "`%ld` - read 1 byte : [%d] - %c" "\x1B[m" "\n", g_tid, client->fd, buff);
		}
...
void idle(struct evcoro_scheduler *scheduler, void *arg)
{
	...
	fprintf(stderr, "\x1B[1;31m" "<<< `%ld` - %ld.%06d : IDLE, tasks [%d]/[%d] >>>" "\x1B[m" "\n",
		g_tid, tv.tv_sec, tv.tv_usec,
		evcoro_workings(scheduler), evcoro_suspendings(scheduler));
...
int main(int argc, char **argv)
{
	...
	if (unlikely(argc != 3)) {
error:
		printf("\x1B[1;31m" "usage %s <ip> <port>" "\x1B[m" "\n", argv[0]);
		return EXIT_FAILURE;
	}
