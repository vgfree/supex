...
#define ASYNC_LIBEV_PEAK        0

#define REDIS_ERR               -1
#define REDIS_OK                0

void redis_callback(struct async_ctx *ac, void *reply, void *data)
{
	printf("redis callback.........\n");
...
int main()
{
	...
	if (ac) {
		...
		rc = conn_xpool_gain(&cpool, "127.0.0.1", 6000, &sfd1);
		...
		if (ok == REDIS_ERR) {
			conn_xpool_push(cpool, &sfd1);
			async_distory(ac);
			return -1;
		}

		char *data = "async_redis_coro!";
		async_command(ac, PROTO_TYPE_REDIS, (int)(intptr_t)sfd1, cpool, redis_callback, data, proto, strlen(proto));
		free(proto);
