...
module('getreleaseadtalksize', package.seeall)
...
function handle()
        ...
        local key
        if args["accountID"]  then
                key = string.format("releaseAdTalk:%s:%s:", args["accountID"], args["appKey"])
                hashkey = args["accountID"]
        else
                key = string.format("releaseAdTalk:%s:%s:", args["imei"], args["appKey"])
                hashkey = args["imei"]
        end
