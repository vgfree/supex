...
module('getreleasemessages', package.seeall)
...
function handle()
        ...
        local hashkey
        local key
        key = string.format("releaseMessage:%s%d",os.date("%Y%m%d%H", args["time"]), os.date("%M", args["time"])/10)
        hashkey = args["time"]
        local curTime = os.time()
        only.log("D", "get releasemessage:" .. key)
        if args["time"] > curTime then
                releasemessage = ""
        elseif curTime - args["time"] <= dk_utils.redisRemainTime then
                ok, releasemessage = dk_utils.smembers_data_from_redis(key, hashkey)
                if not ok then
                        return gosay.resp_msg(msg['MSG_DO_REDIS_FAILED'])
                end
        else
                ok, releasemessage = dk_utils.get_data_from_tsdb(key, args["time"], hashkey)
                if not ok then
                        return gosay.resp_msg(msg['MSG_DO_REDIS_FAILED'])
                end
        end
        --> return result.
        if not releasemessage then
                releasemessage = ""
        end
        only.log("D", "releasemessage: " .. releasemessage)
        -- gosay.go_success(url_tab, msg["MSG_SUCCESS_WITH_RESULT"], releasemessage)
        return gosay.resp_msg(msg["MSG_SUCCESS_WITH_RESULT"], releasemessage)
end 
