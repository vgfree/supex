...
module('getsendbizidsize', package.seeall)
...
local function check_parameter(args)
        ...
        if args["accountID"] and args["imei"] then
...
function handle()
        ...
        local key
        if args["accountID"]  then
                key = string.format("sendMessage:%s:%s:", args["accountID"], args["appKey"])
                hashkey = args["accountID"]
        elseif args["imei"]  then
                key = string.format("sendMessage:%s:%s:", args["imei"], args["appKey"])
                hashkey = args["imei"]
        else
                key = string.format("sendMessage:unknow:%s:", args["appKey"])
                hashkey = args["appKey"]
        end
