...
module('getgeotomessage', package.seeall)
...
function handle()
        ...
        local hashkey
        local key
        key = string.format("GEOtoMessage:%s%d",os.date("%Y%m%d%H", args["time"]), os.date("%M", args["time"])/10)
        hashkey = args["time"]
        local curTime = os.time()
        only.log("D", "get GEOtoMessage:" .. key)
        if args["time"] > curTime then
                geotomessage = ""
        elseif curTime - args["time"] <= dk_utils.redisRemainTime then
                ok, geotomessage = dk_utils.smembers_data_from_redis(key, hashkey)
                if not ok then
                        return gosay.resp_msg(msg['MSG_DO_REDIS_FAILED'])
                end
        else
                ok, geotomessage = dk_utils.get_data_from_tsdb(key, args["time"], hashkey)
                if not ok then
                        return gosay.resp_msg(msg['MSG_DO_REDIS_FAILED'])
                end
        end
        --> return result.
        if not geotomessage then
                geotomessage = ""
        end
        only.log("D", "GEOtoMessage: " .. geotomessage)
        return gosay.resp_msg(msg["MSG_SUCCESS_WITH_RESULT"], geotomessage)
end 
