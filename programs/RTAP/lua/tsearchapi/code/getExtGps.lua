...
module('getExtGps', package.seeall)
...
local function get_data_from_redis(imei, startTime, endTime, gps_tab)
        ...
        for t=startTime, endTime, 600 do
                ...
                -->> key format: RGPS:${imei}:${time_interval}
                key = string.format("RGPS:%s:%s%d", imei, os.date("%Y%m%d%H", t), os.date("%M", t)/10)
...
function handle()
        ...
        if #str == 0 then
                return gosay.resp_msg(msg['MSG_ERROR_REQ_ARG'],'gps count is 0')
        end
