...
extern int      g_update_data_hour;
extern int      g_update_data_min;
extern int      g_update_data_sec;
extern int      g_update_data_cycle;
...
void load_swift_cfg_file(struct swift_cfg_file *p_cfg, char *name)
{
	...
	if (json_object_object_get_ex(cfg, "log_level", &obj)) {
		...
		g_log_level = p_cfg->log_level;				/*业务层日志文件配置信息*/
	} else { goto fail; }

	if (json_object_object_get_ex(cfg, "log_max_files", &obj)) {
		g_log_max = json_object_get_int(obj);			/*业务层日志文件配置信息*/
	} else { goto fail; }

	if (json_object_object_get_ex(cfg, "out_thread_counts", &obj)) {
		g_thread_cnt = json_object_get_int(obj);		/*启动业务线程数量*/
	} else { goto fail; }

	if (json_object_object_get_ex(cfg, "update_data_hour", &obj)) {
		g_update_data_hour = json_object_get_int(obj);
	} else { goto fail; }

	if (json_object_object_get_ex(cfg, "update_data_min", &obj)) {
		g_update_data_min = json_object_get_int(obj);
	} else { goto fail; }

	if (json_object_object_get_ex(cfg, "update_data_sec", &obj)) {
		g_update_data_sec = json_object_get_int(obj);
	} else { goto fail; }

	if (json_object_object_get_ex(cfg, "update_data_cycle", &obj)) {
		g_update_data_cycle = json_object_get_int(obj);
	} else { goto fail; }
...
fail:
	dprintf(2, "invalid config file :%s\n", name);
	exit(EXIT_FAILURE);
}
