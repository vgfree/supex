...
int alive_vms_call(void *user, union virtual_system **VMS, struct adopt_task_node *task)
{
	...
	switch (op_type)
	{
		case 0x00:	// 握手
			...
			if (mfptpSvp_libkv_check_handshake(task->cid, task->sfd, "1", 1) == 0) {
			...
			if (mfptpSvp_redis_check_gpstoken(mirrtalk_id, gps_token, strlen(gps_token)) < 0) {
			...
			mfptpSvp_redis_del_gpstoken(mirrtalk_id);

			mfptpSvp_libkv_set(task->cid, task->sfd, "1");
...
ERROR:
	mfptpSvp_libkv_del(task->cid, task->sfd);
...
{
	mfptpSvp_libkv_set(task->cid, task->sfd, "0");
	return 0;
}
