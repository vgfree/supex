...
--创建者：王盼盼
--创建日期：2015/12/01
--文件描述：从redis中查询得到所需的content 和 aid
...
local function get_citycode(Cid,lat,lng,appKey)
        ...
        if not ret_body then
            ...
            local string = '{"result":"nok", "error":"http request  do failed!"}' .. '\n'
        ...
        if body then
            ...
            if ok and jo then
                if jo["ERRORCODE"]  == "ME24902" then
                    ...
                    local afp = supex.rgs(200)
                    local string = '{"result":"nok", "error":"http body do failed!"}' .. '\n'
                    supex.say(afp, string)
                    return supex.over(afp)
...
function AdCube_get(Cid, lat, lng, typ, appKey, sign)
    if not Cid or not lat or not lng or not typ or not appKey or not sign then
    ...
    if not ret then
        ...
        if not ok or not id_info then
            ...
            local afp = supex.rgs(200)
            local string = '{"result":"nok", "error":"redis id_info do failed!"}' .. '\n'
            supex.say(afp, string)
            return supex.over(afp)
        end
        ...
        for i=1, #id_info do
            ...
            if not ok or not value_tab  then
                ...
                local afp = supex.rgs(200)
                local string = '{"result":"nok", "error":"redis value_tab do failed!"}' .. '\n'
                supex.say(afp, string)
                return supex.over(afp)
            end
                ...
                if typ == ctype then
                    ...
                    for s=1,#str1 do
                        ...
                        if (time1 <= time) and (time <= time2) then
                            local value = {id = id_info[i], content = Content, url = Url, citycode = citycode}
        ...
        if not id_content then
            ...
            local afp = supex.rgs(200)
            local string = '{"result":"nok", "error":"content does not exist!"}' .. '\n'
            supex.say(afp, string)
            return supex.over(afp)
        end
        ...
        --    local status = "ok"
        --    local time = os.time()
        --    CB.cb(id_content['id'], Cid, status, time, appKey, sign,  id_content['url'])    

        local ok, ret = pcall(cjson.encode, id_content)
        if not ok or not ret then
            only.log("E", Cid .. " redis ret do failed!\n")
            local afp = supex.rgs(200)
            local string = '{"result":"nok", "error":"redis ret do failed!"}' .. '\n'
            supex.say(afp, string)
            return supex.over(afp)
        end      
...
function handle()
    ...
    only.log("D","cid ="..tostring(cid).."lat ="..tostring(lat).."lng ="..tostring(lng).."typ ="..tostring(typ).."appKey ="..tostring(appKey).."sign ="..tostring(sign))
    AdCube_get(cid, lat, lng, typ, appKey, sign)
end
