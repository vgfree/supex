...
function DataPackage:new(IMEI, accountID, tokenCode)
	local self = {
	}
...
function DataPackage:get_last_point()
	--TODO luakv
	local last_point_key = string.format("%s:lastPoint", self['IMEI'])
	local ok, ret = luakv_api.cmd('localRedis', self['IMEI'] or '',  "hmget", last_point_key, 
	'GPSTime',
	'longitude',
	'latitude',
	'direction',
	...
	last_point:init({GPSTime=ret[1],longitude=ret[2],latitude=ret[3],direction=ret[4],speed=ret[5], altitude=ret[6]})
...
function DataPackage:set_last_point(last_point)
	--TODO luakv
	local last_point_key = string.format("%s:lastPoint", self['IMEI'])
	local ok,_ = luakv_api.cmd('localRedis', self['IMEI'] or '',  "hmset", last_point_key, 
...
function DataPackage:checkLastTokenCode()
	--TODO cachekv
	local token_key = string.format("%s:tokenCode", self['IMEI'])	
	--获取上次tokenCode
	local ok,last_tokenCode = cachekv.pull('owner', self['IMEI'] or '',  "get", token_key)
	...
	if not ok or not last_tokenCode then
		...
		--保存当前tokenCode
		local ok,_ = cachekv.push('owner', self['IMEI'] or '',  "set", token_key, self['tokenCode'])
		if not ok then
			only.log('E', "set last tokenCode failed!,IMEI:%s, tokenCode:%s", self['IMEI'], self['tokenCode'])
		end
	else
...
function DataPackage:parseBody(points, gps_body, extra_flag)
	...
	repeat
		...
		p_gps:init({GPSTime=time_array[i],longitude=lon_array[i],latitude=lat_array[i],
		speed=speed_array[i],direction=dir_array[i],altitude=alt_array[i],isExtra=extra_flag})
		--table.insert(points,p_gps)
