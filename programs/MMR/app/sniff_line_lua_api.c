...
static int _vms_call_1(lua_State **L, int last, struct sniff_task_node *task)
{
...
static int _vms_call_2(lua_State **L, int last, struct sniff_task_node *task)
{
...
{
	return sniff_for_alone_vm(user, task, (SNIFF_VMS_FCB)_vms_call_2, (SNIFF_VMS_ERR)_vms_erro);
}
 ...
 */
static int _vms_monitor(lua_State **L, int last, struct sniff_task_node *task)
{
 ...
 * 修 改:新添加函数　 程少远　2015/05/12
 */
int sniff_vms_monitor(void *user, void *task)
{
	SNIFF_WORKER_PTHREAD    *p_sniff_worker = (SNIFF_WORKER_PTHREAD *)user;
	struct sniff_task_node  *p_task = task;

	p_task->sfd = p_sniff_worker->index;	/*reuse*/
	return sniff_for_alone_vm(user, task, (SNIFF_VMS_FCB)_vms_monitor, (SNIFF_VMS_ERR)_vms_erro);
}
