...
static void copy_swift_cfg(struct swift_cfg_file *dest, struct swift_cfg_file *src)
{
	...
	if (src->ptype == USE_HTTP_PROTO) {
		if (src->api_fetch) {
			dest->api_fetch = x_strdup(src->api_fetch);
		}

		if (src->api_merge) {
			dest->api_merge = x_strdup(src->api_merge);
		}

		if (src->api_apply) {
			dest->api_apply = x_strdup(src->api_apply);
		}

		memcpy(dest->api_names, src->api_names, sizeof(src->api_names));
	}
}
...
static void free_swift_cfg(struct swift_cfg_file *p_cfg)
{
	...
	if (p_cfg->ptype == USE_HTTP_PROTO) {
		...
		memset(p_cfg->api_names, 0, sizeof(p_cfg->api_names));
	}
}
...
bool reload_swift_cfg_file(struct swift_cfg_file *p_cfg, char *name)
{
