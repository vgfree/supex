...
function GPSFrame:new(IMEI, tokenCode, startTime, endTime, fileName)
	local self = {
	}
...
function GPSFrame:calcMiles(prev_gps, next_gps)
	...
	elseif (time2 - time2) > 5 then	--时间间隔大于5秒 不计算
		...
		time1,time2,speed1,speed2,a,s = nil,nil,nil,nil,nil,nil
...
function GPSFrame:calcPath(data)
	...
	for i=st_idx, ed_idx do
		repeat
			...
			--获取道路信息
			roadrecord:getRoadInfo(self['IMEI'], self['tokenCode'], gps_point)
	...
	st_idx,ed_idx,roadrecord,gps_point,last_point,premileage = nil,nil,nil,nil,nil,nil
end

--功  能: 将recordSet写入文件
--
function GPSFrame:writeToFile(fileName)
...
function GPSFrame:process()
	...
	only.log('D', string.format("IMEI:%s, tokenCode:%s, frameStart:%s, frameEnd:%s", 
		self['IMEI'], self['tokenCode'], self['frameStart'], self['frameEnd']))
	...
	if not next(data) then
		only.log('W', string.format("merge gps data is none! IMEI:%s, tokenCode:%s", 
