	...
	QUEUE_ENTITY_CALL                       minor_pull)
{
	...
	free_queue_init(&p_stat->swap, task_size, MAX_SWITCH_QUEUE_SWAP_SIZE);
		...
		case 1:
			...
			if (ok) {
				if (p_node->base->shift == 1) {
					assert(free_queue_push(&p_stat->swap, p_node->data));	// push l
					p_stat->step_lookup++;
					ok = false;
				} else {
			...
			if (p_stat->mark_lookup == MARK_USE_MAJOR) {
				...
				if (ok) {
					if (p_node->base->shift == p_stat->shift_lookup) {
						...
						ok = free_queue_pull(&p_stat->swap, p_node->data);	// pull l
						...
						assert(free_queue_push(&p_stat->swap, p_stat->temp.data));	// push l
						p_stat->shift_lookup++;
