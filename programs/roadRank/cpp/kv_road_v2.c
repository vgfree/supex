...
{               
        ...
        sprintf(sub, "%lf:%lf:%d:%ld:%d:%d;", road->road_sec[98].longitude, road->road_sec[98].latitude, road->road_sec[98].avg_speed, road->road_sec[98].endtime, road->road_sec[98].max_speed, road->road_sec[98].used_time);
        strcat(ret, sub);
                
        for( i = 0; i < road->sec_num; i++ ) {
                memset( sub, 0, sizeof(sub) );
                sprintf(sub, "%lf:%lf:%d:%ld:%d:%d;", road->road_sec[i].longitude, road->road_sec[i].latitude, road->road_sec[i].avg_speed, road->road_sec[i].endtime, road->road_sec[i].max_speed, road->road_sec[i].used_time);
                strcat(ret, sub);
        }       
                
        memset( sub, 0, sizeof(sub) );
        sprintf(sub, "%lf:%lf:%d:%ld:%d:%d;", road->road_sec[99].longitude, road->road_sec[99].latitude, road->road_sec[99].avg_speed, road->road_sec[99].endtime, road->road_sec[99].max_speed, road->road_sec[99].used_time);
        strcat(ret, sub);
}               
