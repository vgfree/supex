...
int get_IMEI_from_kv(long long IMEI, KV_IMEI *kv_IMEI)
{
	char            buff[10240] = { 0 };
	kv_answer_t     *ans;
	int             flag = SUC_IMEI;
	...
	sprintf(buff, "hmget %lld count max_speed_num max_speed str_time end_time roadID IMEI citycode countycode direction rt start_lon start_lat end_lon end_lat", IMEI);
	char kv_hash[128];
	sprintf(kv_hash, "%lld", IMEI);
...
int set_IMEI_to_kv(KV_IMEI *kv_IMEI)
{
	...
	sprintf(buff,
		"hmset %lld count %d max_speed_num %d max_speed %d str_time %ld end_time %ld roadID %ld IMEI %lld citycode %ld countycode %ld direction %d rt %d start_lon %f start_lat %f end_lon %f end_lat %f",
