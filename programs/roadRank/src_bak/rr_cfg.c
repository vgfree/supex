...
bool read_rr_cfg(struct rr_cfg_file *p_cfg, char *name)
{
	...
	// min_speed_limit
	if (!json_object_object_get_ex(cfg, "min_speed_limit", &obj)) {
		x_printf(E, "can't found min_speed_limit!\n");
		goto fail;
	}

	p_cfg->min_speed_limit = (int)json_object_get_int(obj);

	// road_match_limit
	if (!json_object_object_get_ex(cfg, "road_match_limit", &obj)) {
		x_printf(E, "can't found road_match_limit!\n");
		goto fail;
	}

	p_cfg->road_match_limit = (int)json_object_get_int(obj);
