...
int get_IMEI_from_kv(char *IMEI, KV_IMEI *kv_IMEI)
{
	...
	if (len != 1) {
		...
		while ((value = answer_iter_next(iter)) != NULL) {
			...
			{
				case 1:
					kv_IMEI->count = atoi(ptr); break;

				case 2:
					kv_IMEI->max_speed_num = atoi(ptr); break;

				case 3:
					kv_IMEI->max_speed = atoi(ptr); break;

				case 4:
					kv_IMEI->str_time = atoll(ptr); break;

				case 5:
					kv_IMEI->end_time = atoll(ptr); break;

				case 6:
					kv_IMEI->roadID = atoll(ptr); break;

				case 7:
					strcpy(kv_IMEI->IMEI, ptr); break;

				case 8:
					kv_IMEI->citycode = atoll(ptr); break;

				case 9:
					kv_IMEI->countycode = atoll(ptr); break;

				case 10:
					kv_IMEI->direction = atoi(ptr); break;

				case 11:
					kv_IMEI->rt = atoi(ptr); break;

				case 12:
					kv_IMEI->start_lon = strtod(ptr, NULL); break;

				case 13:
					kv_IMEI->start_lat = strtod(ptr, NULL); break;

				case 14:
					kv_IMEI->end_lon = strtod(ptr, NULL); break;

				case 15:
					kv_IMEI->end_lat = strtod(ptr, NULL); break;
