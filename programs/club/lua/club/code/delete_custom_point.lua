...
module('delete_custom_point', package.seeall)
...
function handle()
	...
	for k,v in ipairs(point_info) do
		...
		local grid_lon,grid_lat = PUBLIC_FUNC_LIST.grid_five_thousandths(lon,lat)
                ...
                local ok,data = luakv_api.cmd('FiveThousandths'," ",'hdel',grid,v['id'])
                if not ok then
			...
			PUBLIC_FUNC_LIST.send_respond(str)
                ...
                local ok = redis_api.cmd('clubCustomPoint'," ",'hdel',grid,v['id'])
                if not ok then
