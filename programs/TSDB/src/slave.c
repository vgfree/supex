...
static int32_t send_request(const char *msg, size_t len)
{
	...
	while (len > 0) {
		...
		if (ret < 0) {
			x_printf(E, "sync seq[%ld] error: send error, errno[%d]", binlog->cur_seq, errno);
			return -1;
		}
...
static int recv_response()
{
	while (1) {
		...
		if (ret == 0) {
			x_printf(E, "recv error: remote socket closed, seq:[%ld]", binlog->cur_seq);
			return -1;
		}
		...
		x_printf(E, "error: recv error: remote socket broken, seq:[%ld]", binlog->cur_seq);
		return -1;
	}
	...
	while (!slave->is_quit) {
		if (slave->sock == -1) {
			...
			if (sock_connect(slave->ip, slave->port) == -1) {
				x_printf(E, "socket connect ip:[%s], port:[%d] failed.", slave->ip, slave->port);
				sleep(5);
				continue;
			}

			x_printf(I, "socket connect ip:[%s], port:[%d] success.", slave->ip, slave->port);
		}
...
heart_beat:
		...
		if (send_request("*1\r\n$4\r\nPING\r\n", strlen("*1\r\n$4\r\nPING\r\n")) != 0) {
			x_printf(E, "heart beat failure: slave maybe disconnect.");
		...
		if (check_response("PONG") != 0) {
			x_printf(E, "recv error msg, then send again. req: [%ld], send_buf:[%s].", binlog->cur_seq, send_buf);
			goto heart_beat;
		}
		...
		while (binlog->cur_seq <= binlog->last_seq) {
			...
			if (ret == -1) {
				x_printf(E, "get binlog req:[%ld] failed.", binlog->cur_seq);
				continue;
			} else if (ret == 0) {
...
delete_binlog:
			...
			if (ret == -1) {
				x_printf(E, "delete binlog req:[%ld] failed, may need to manually synchronize it.", binlog->cur_seq);
			}
...
int slave_open(struct _leveldb_stuff *ldbs, uint8_t is_slave, const char *ip, uint16_t port)
{
	...
	if (ret != 0) {
		x_printf(F, "create slave thread error. errno: %d", errno);
		return -1;
	}
