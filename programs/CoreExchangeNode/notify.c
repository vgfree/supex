	...
	struct comm_tcp *portinfo, void *usr)
{
	...
	switch (portinfo->stat)
	{
		...
		case FD_CLOSE:
		{
			fdman_array_remove_fd(portinfo->fd);
	...
	struct comm_tcp *portinfo, void *usr)
{
	if (g_serv_info.commctx != commctx) {
		x_printf(E, "callback commctx not equal. g_serv_info.commctx:%p, commctx:%p",
	...
	switch (portinfo->stat)
	{
		...
		{
			struct fd_descriptor des;
			fdman_array_at_fd(portinfo->fd, &des);
			x_printf(D, "fdman_array_at_fd, status:%d, obj:%d.", des.status, des.obj);

			if (des.status != 1) {
				x_printf(E, "this fd:%d is not running.", portinfo->fd);
				return;
			}
			...
			if (g_serv_info.message_gateway_fd == portinfo->fd) {
				...
				x_printf(E, "this fd:%d is not belong to MESSAGE_GATEWAY.", portinfo->fd);
			}
	...
	switch (portinfo->stat)
	{
		...
		case FD_CLOSE:	// closed.
		{
			...
			if (g_serv_info.setting_server_fd == portinfo->fd) {
				...
				x_printf(E, "this fd:%d is not belong to SETTING_SERVER.", portinfo->fd);
			}
	...
	switch (portinfo->stat)
	{
		...
		case FD_CLOSE:	// closed.
		{
			...
			if (g_serv_info.login_server_fd == portinfo->fd) {
				...
				x_printf(E, "this fd:%d is not belong to LOGIN_SERVER.", portinfo->fd);
			}
