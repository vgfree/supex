...
void case_cmd_pexpireat_get()
{
        ...
        int write = snprintf(cmd, sizeof(cmd), "pexpireat key %ld", (time(NULL) + 2)*1000);
...
void case_cmd_pexpireat_invalid_cmd()
{
        ...
        handler = kv_create(NULL);
        ans = kv_ask(handler, "pexpirea key 10", strlen("pexpirea key 10"));
...
void case_cmd_pexpireat_invalid_param()
{
        ...
        ans = kv_ask(handler, "pexpireat", strlen("pexpireat"));
        CU_ASSERT_EQUAL(ans->errnum, ERR_ARGUMENTS);
        CU_ASSERT_PTR_EQUAL(kv_answer_value_to_string(kv_answer_first_value(ans)), NULL);
        kv_answer_release(ans); 
        
        ans = kv_ask(handler, "pexpireat key", strlen("pexpireat key"));
        CU_ASSERT_EQUAL(ans->errnum, ERR_ARGUMENTS);
        CU_ASSERT_PTR_EQUAL(kv_answer_value_to_string(kv_answer_first_value(ans)), NULL);
        kv_answer_release(ans);

        ans = kv_ask(handler, "pexpireat key 10 20", strlen("pexpireat key 10 20"));
        CU_ASSERT_EQUAL(ans->errnum, ERR_ARGUMENTS);
        CU_ASSERT_PTR_EQUAL(kv_answer_value_to_string(kv_answer_first_value(ans)), NULL);
        kv_answer_release(ans);

        ans = kv_ask(handler, "pexpireat key 123456789012345678901234567890", strlen("pexpireat key 123456789012345678901234567890"));
...
void case_cmd_pexpireat_multi_handler()
{
        ...
        snprintf(buf, sizeof(buf), "pexpireat keyA %ld", (time(NULL)+1)*1000);
        ans = kv_ask(handlerA, buf, strlen(buf));
        CU_ASSERT_EQUAL(ans->errnum, ERR_NONE);
        CU_ASSERT_STRING_EQUAL(kv_answer_value_to_string(kv_answer_first_value(ans)), "1");
        kv_answer_release(ans);

        snprintf(buf, sizeof(buf), "pexpireat keyB %ld", (time(NULL)+3)*1000);
        ans = kv_ask(handlerB, buf, strlen(buf));
        CU_ASSERT_EQUAL(ans->errnum, ERR_NONE);
        CU_ASSERT_STRING_EQUAL(kv_answer_value_to_string(kv_answer_first_value(ans)), "1");
        kv_answer_release(ans);

        snprintf(buf, sizeof(buf), "pexpireat keyC %ld", (time(NULL)+5)*1000);
