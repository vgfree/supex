...
void case_cmd_lpush_add_few_bytes()
{
        ...
        ans = kv_ask(handler, "lpush lkey a", strlen("lpush lkey a"));
        CU_ASSERT_EQUAL(ans->errnum, ERR_NONE);
        CU_ASSERT_STRING_EQUAL(kv_answer_value_to_string(kv_answer_first_value(ans)), "1");
        kv_answer_release(ans);

        ans = kv_ask(handler, "lpush lkey b", strlen("lpush lkey b"));
        CU_ASSERT_EQUAL(ans->errnum, ERR_NONE);
        CU_ASSERT_STRING_EQUAL(kv_answer_value_to_string(kv_answer_first_value(ans)), "2");
        kv_answer_release(ans);

        ans = kv_ask(handler, "lpush lkey c", strlen("lpush lkey c"));
        CU_ASSERT_EQUAL(ans->errnum, ERR_NONE);
        CU_ASSERT_STRING_EQUAL(kv_answer_value_to_string(kv_answer_first_value(ans)), "3");
        kv_answer_release(ans);

        ans = kv_ask(handler, "lpush lkey d e f g h", strlen("lpush lkey d e f g h"));
        CU_ASSERT_EQUAL(ans->errnum, ERR_NONE);
        CU_ASSERT_STRING_EQUAL(kv_answer_value_to_string(kv_answer_first_value(ans)), "8");
        kv_answer_release(ans);
...
void case_cmd_lpush_add_lot_bytes()
{
        ...
        for (i = 1; i <= 1000; i++) {
                ...
                ans = kv_ask(handler, cmd, dc[i-1]->ptrlen + strlen("lpush lkey "));
                CU_ASSERT_EQUAL(ans->errnum, ERR_NONE);
                kv_answer_release(ans);
        }
...
void case_cmd_lpush_del()
{
        ...
        ans = kv_ask(handler, "lpush lkey 123 456", strlen("lpush lkey 123 456"));
        CU_ASSERT_EQUAL(ans->errnum, ERR_NONE);
        kv_answer_release(ans);
...
void case_cmd_lpush_invalid_param()
{
        ...
        ans = kv_ask(handler, "lpush", strlen("lpush"));
        CU_ASSERT_EQUAL(ans->errnum, ERR_ARGUMENTS);
        kv_answer_release(ans);

        ans = kv_ask(handler, "lpush lkey", strlen("lpush lkey"));
        CU_ASSERT_EQUAL(ans->errnum, ERR_ARGUMENTS);
        kv_answer_release(ans);
...
void case_cmd_lpush_stress()
{
        ...
        sprintf(buf, "lpush %s value", longkey->ptr);
        ans = kv_ask(handlerA, buf, strlen(buf));
        CU_ASSERT_EQUAL(ans->errnum, ERR_NONE);
        CU_ASSERT_STRING_EQUAL(kv_answer_value_to_string(kv_answer_first_value(ans)), "1");
        kv_answer_release(ans);
        ...
        for (i = 0; i < 1000; i++) {
                memcpy(buf, "lpush ", strlen("lpush "));
                memcpy(buf + strlen("lpush "), test_data[i]->ptr, test_data[i]->ptrlen);
                memcpy(buf + strlen("lpush ") + test_data[i]->ptrlen, " ", 1);
                memcpy(buf + strlen("lpush ") + test_data[i]->ptrlen + 1, test_data[i]->ptr, test_data[i]->ptrlen);
                memcpy(buf + strlen("lpush ") + test_data[i]->ptrlen + 1 + test_data[i]->ptrlen, "\0", 1);
