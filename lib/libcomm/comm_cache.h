...
/*初始化cache*/
bool commcache_init(struct comm_cahce* comm_cahce,  int capacity);

/*往cache里面添加datasize大小的数据*/
bool commcache_append(struct comm_chache* comm_cahce,  const char* data,  int datasize);

/*减少cache有效数据的计数  size:解析的数据的多少*/
void commcache_deccnt(struct comm_cahce* comm_cache, int size);
