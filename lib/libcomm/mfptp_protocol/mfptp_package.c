...
void  mfptp_packager_init(struct mfptp_packager *packager, char *buff, int *size)
{
...
int mfptp_packager(struct mfptp_packager *packager, const char *data, unsigned char flag, int method)
{
	...
	packager->header.compression = flag & (0x0F << 4);
	...
	for(i = 0; i < packages; i++) {
		for(j = 0; j < frames_of_pack[i]; j++, k++) {
			...
			if (unlikely(k-1 == frames)) {
				packager->package.frame[i].frame_size[j] = dsize - frame_offset[k];
			} else {
				packager->package.frame[i].frame_size[j] = frame_offset[k+1] - frame_offset[k];
			}
		}
		packager->package.frame[i].frames = frames_of_pack[i];
	}
	packager->package.dsize = dsize;
	packager->package.packages = packages;
	packager->package.frames = frames;
}
