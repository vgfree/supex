		...
		switch ((int)parser->ms.step)
		{
			...
			case MFPTP_HEAD_LAST:
				if ((*parser->ms.data)[parser->ms.dosize] == 'P') {
					...
					loger("bad protocol last letter wrong\n");
					parser->ms.error = MFPTP_HEAD_INVAILD;
				}
				dsize -= 1;
				parser->ms.dosize += 1;
