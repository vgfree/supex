...
// ADD YGL DATA
CDMSysSource *g_pcDMSysSource = NULL;
// FUNCTIONS
...
{
	// ?????为什么没有添加QTSS_RTPSendPackets_Role
...
{
	qtss_printf("********** [QTSSFileModule] Initialize **********\n");

	QTRTPFile::Initialize();// FileModle 初始化的时候就要初始化RTPFile的全局Init函数
	QTSSModuleUtils::Initialize(inParams->inMessages, inParams->inServer, inParams->inErrorLogStream);
	QTSS3GPPModuleUtils::Initialize(inParams);

	// FileModule 模块保存服务端信息
	sPrefs = QTSSModuleUtils::GetModulePrefsObject(inParams->inModule);
	sServerPrefs = inParams->inPrefs;
	sServer = inParams->inServer;

	// Read our preferences
	// 从easydarwin.xml配置文件中读出FileModule模块的所有配置信息
	RereadPrefs();
	...
	// ygl add 2016-01-08
	if (NULL == g_pcDMSysSource) {
		g_pcDMSysSource = new CDMSysSource();
	}

	if (NULL != g_pcDMSysSource) {
		g_pcDMSysSource->InitDMSysClient(Socket::kNonBlockingSocketType);
		g_pcDMSysSource->SetDMSysClientInfo(g_pcDMSysSource->GetHostAddr(), g_pcDMSysSource->GetHostPort(), g_pcDMSysSource->GetLocalAddr());
	}
...
void BuildPrefBasedHeaders()
{
	...
	qtss_printf("********** [FileModule] BuildHeaders **********\n");
	sURLHeader.PrintStr();
	sEmailHeader.PrintStr();
...
QTSS_Error ProcessRTSPRequest(QTSS_StandardRTSP_Params *inParamBlock)
{
	...
	if ((QTSS_GetValuePtr(inParamBlock->inRTSPRequest, qtssRTSPReqMethod, 0,
		(void **)&theMethod, &theMethodLen) != QTSS_NoErr) || (theMethodLen != sizeof(QTSS_RTSPMethod))) {	// RTSP请求方法错误情况校验
		Assert(0);
		return QTSS_RequestFailed;
	}

	qtss_printf("***** [QTSSFileModule] ProcessRTSPRequest method:%d *****\n", *theMethod);
...
Bool16 isSDP(QTSS_StandardRTSP_Params *inParamBlock)
{
	...
	// 获取到path值[从RTSPRequest字典中获取属性qtssRTSPReqLocalPath的值]
	QTSS_Error theErr = QTSS_GetValuePtr(inParamBlock->inRTSPRequest, qtssRTSPReqLocalPath, 0, (void **)&path, &len);	// rtsp request local path
	Assert(theErr == QTSS_NoErr);

	qtss_printf("***** [QTSSFileModule] isSDP path:%s. *****\n", path);
...
{
	qtss_printf("***** [QTSSFileModule] DoDescribe *****\n");

	if (isSDP(inParamBlock)) {	// .sdp格式文件处理
		StrPtrLen pathStr;

		// 使用前先Lock
		(void)QTSS_LockObject(inParamBlock->inRTSPRequest);

		(void)QTSS_GetValuePtr(inParamBlock->inRTSPRequest, qtssRTSPReqFilePath, 0, (void **)&pathStr.Ptr, &pathStr.Len);	// rtsp request file path
	...
	// 从ClientSession字典中得到sFileSessionAttr值
	(void)QTSS_GetValue(inParamBlock->inClientSession, sFileSessionAttr, 0, (void *)&theFile, &theLen);	// fielsession对象

	// Generate the complete file path
	UInt32                  thePathLen = 0;
	OSCharArrayDeleter      thePath(QTSSModuleUtils::GetFullPath(inParamBlock->inRTSPRequest, qtssRTSPReqFilePath, &thePathLen, &sSDPSuffix));	// file path
	// 出来后thepath是以.sdp结尾

	// first locate the target movie//requestpath截断.sdp尾
	...
	// requestPath.PrintStr();
	// qtss_printf("***** [QTSSFileModule] , len:%d\n", requestPath.Len);
	...
	if (theFile != NULL) {	// fileSession已存在
		...
		qtss_printf("***** [QTSSFileModule] there is already a file:%s for this session, new request file:%s. *****\n", moviePath.GetAsCString(), requestPath.GetAsCString());
		...
		if (!requestPath.Equal(moviePath)) {	// 已存在FileSession与新请求不相同
	...
	if (theFile == NULL) {
		...
		if (theErr != QTSS_NoErr) {	// 新建QTRTPFile失败
			return theErr;
		}
	...
	// StrPtrLen strtest(thePath.GetObject(), ::strlen(thePath.GetObject()));// 测试打印
	...
	if (theSDPData.Len > 0) {
		...
		if (sRecordMovieFileSDP && !pathEndsWithSDP) {		// don't auto create sdp for an sdp file because it would look like a broadcast
			qtss_printf("***** [QTSSFileModule] DoDescribe Record Movie SDP file. *****\n");
		...
		(void)QTSS_GetValueAsString(inParamBlock->inRTSPRequest, qtssRTSPReqFilePath, 0, &fileNameStr);	// file path
		qtss_printf("***** [QTSSFileModule] DoDescribe filename:%s.\n\n", fileNameStr);
		...
		// qtss_printf("***** [QTSSFileModule] file sdp data:%s .\n\n", theSDPData.Ptr);
...
{
	qtss_printf("***** [QTSSFileModule] CreateQTRTPFile path:%s *****\n", inPath);
	*outFile = NEW FileSession();

	(*outFile)->fFile.SetAllowInvalidHintRefs(sAllowInvalidHintRefs);
	QTRTPFile::ErrorCode theErr = (*outFile)->fFile.Initialize(inPath);	// 初始化RTPFile
...
{
	qtss_printf("***** [QTSSFileModule] DoSetup *****\n");
	...
	if ((theErr != QTSS_NoErr) || (theLen != sizeof(FileSession *))) {	// GetValue获取失败情况
		char *theFullPath = NULL;
		// theErr = QTSS_GetValuePtr(inParamBlock->inRTSPRequest, qtssRTSPReqLocalPath, 0, (void**)&theFullPath, &theLen);
		theErr = QTSS_GetValueAsString(inParamBlock->inRTSPRequest, qtssRTSPReqLocalPath, 0, &theFullPath);	// 获取rtsp request localpath
		Assert(theErr == QTSS_NoErr);
...
QTSS_Error DoPlay(QTSS_StandardRTSP_Params *inParamBlock)
{
	...
	qtss_printf("***** [QTSSFileModule] DoPlay *****\n");
	...
	if (theErr == QTSS_NoErr) {
		...
		if ((theErr != QTSS_NoErr) || (theLen != sizeof(Float64))) {	// No start time so just start at the last packet ready to send
...
{
	qtss_printf("***** [QTSSFileModule] SendPackets *****\n");
...
{
	qtss_printf("***** [QTSSFileModule] DestroySession. *****\n");
...
// TCPSocket CDMSysClient::m_cClientSocket;

CDMSysClient::CDMSysClient(CDMSysSocket *insocket) : m_pcDMSysSocket(insocket)
						     , m_iDMSysIPCfg(0), m_iDMSysPortCfg(0), m_bConnect(false) {}

void CDMSysClient::InitDMSysClient()
{
	qtss_printf("[QTSSFileModule] CDMSysClient InitDMSysClient. \n");
	// 从配置文件中获取DMSystem的IP和port
	m_iDMSysIPCfg = ntohl(inet_addr("192.168.11.222"));	// 网络序
	m_iDMSysPortCfg = 560;

	// m_pcDMSysSocket->Set(m_iDMSysIPCfg, m_iDMSysPortCfg);
	// 获取本机localaddr
	UInt32 localaddr = m_iDMSysIPCfg;
}

void CDMSysClient::Connect2DMSys()
{
	char *ip = NULL;

	qtss_printf("[QTSSFileModule] CDMSysClient Connect2DMSys. ip:%d, port:%d \n", m_iDMSysIPCfg, m_iDMSysPortCfg);

	if (NULL != m_pcDMSysSocket) {
		// m_pcDMSysSocket->Connect(m_pcDMSysSocket->GetSocket());
	}
	 ...
	 *   if(localsocket < 0){
	 *   qtss_printf("[QTSSFileModule] DMSysInterface Connect2DMSys create local socket error.\n");
	 *   m_bConnect = false;
	 *   return;
	 *   }
	 *
	 *   if(bind(localsocket, (struct sockaddr*)&localaddr, sizeof(localaddr))){
	 *    qtss_printf("[QTSSFileModule] DMSysInterface Connect2DMSys bind socket error.\n");
	 *    m_bConnect = false;
	 *    return;
	 *   }
	 *
	 ...
	 *   remoteaddr.sin_addr.s_addr = m_iDMSysIP;
	 *   remoteaddr.sin_port = htons(m_iDMSysPort);
	 *
	 *   socklen_t remoteaddrlen = sizeof(remoteaddr);
	 *   if(connect(localsocket,(struct sockaddr*)&remoteaddr, remoteaddrlen) < 0){
	 *    qtss_printf("[QTSSFileModule] DMSysInterface Connect2DMSys connect socket error.\n");
	 *    m_bConnect = false;
	 *    return;
	 *   }
	 *
	 *   // 连接成功，以后向该socket发送数据
	 *   m_iLocalSocket = localsocket;
	 *   m_bConnect = true;
	 *
	 *   //测试发送数据
	 *   char buff[1024] = {0};
	 *   char* tests = "tiantiantiannan";
	 *   qtss_sprintf(buff, tests);
	 *   send(m_iLocalSocket, buff, 1024, 0);
	 *   qtss_sprintf(buff, "%s", "secondsenddata");
	 *   send(m_iLocalSocket, buff, 1024, 0);
	 *   //recv(m_iLocalSocket, buff, 1024, 0);
	 */
}

void CDMSysClient::SendData()
{
	qtss_printf("[QTSSFileModule] CDMSysClient SendData.\n");
}

OS_Error CDMSysClient::ReceiveResponse()
{}

///////////////////////CDMSysSocket 实现///////////////////
CDMSysSocket::CDMSysSocket()
	: fHostAddr(0),
	  fHostPort(0),
	  fEventMask(0),
	  fSocketP(NULL),
	  ...
	  fSentLength(0)
{
	// qtss_printf("********** CDMSysSocket::CDMSysSocket **********\n");
}

OS_Error CDMSysSocket::Open(TCPSocket *inSocket)
{
	OS_Error theErr = OS_NoErr;

	if (!inSocket->IsBound()) {	// SOCKET UNBOUNDED
		qtss_printf("[CDMSysSocket] Open socket\n\n");
...
OS_Error CDMSysSocket::Connect(TCPSocket *inSocket)
{
	qtss_printf("[CDMSysSocket] Connect socket:%p\n", inSocket);
	OS_Error theErr = this->Open(inSocket);

	Assert(theErr == OS_NoErr);

	if (theErr != OS_NoErr) {	// OPEN FAILED
		return theErr;
	}

	if (!inSocket->IsConnected()) {	// SOCKET is not connect state active connect
...
OS_Error CDMSysSocket::Send(char *inData, const UInt32 inLength)
{
	iovec theVec[1];
...
OS_Error CDMSysSocket::SendSendBuffer(TCPSocket *inSocket)
{
	OS_Error        theErr = OS_NoErr;
...
///////////////////////////////CTCPDMSysSocket 实现//////////////////////
CTCPDMSysSocket::CTCPDMSysSocket(UInt32 inSocketType)
	: fSocket(NULL, inSocketType)
{
	fSocketP = &fSocket;
	this->Open(&fSocket);	// 构造时就打开本地socket
}

// 设置socket属性
void CTCPDMSysSocket::SetOptions(int sndBufSize, int rcvBufSize)
{
	qtss_printf("CTCPDMSysSocket::SetOptions sndBufSize=%d, rcvBuf=%d\n", sndBufSize, rcvBufSize);

	int err = 0;
	err = ::setsockopt(fSocket.GetSocketFD(), SOL_SOCKET, SO_SNDBUF, (char *)&sndBufSize, sizeof(int));	// sndbuff
	AssertV(err == 0, OSThread::GetErrno());

	err = ::setsockopt(fSocket.GetSocketFD(), SOL_SOCKET, SO_RCVBUF, (char *)&rcvBufSize, sizeof(int));	// rcvbuff
	AssertV(err == 0, OSThread::GetErrno());
...
OS_Error CTCPDMSysSocket::SendV(iovec *inVec, UInt32 inNumVecs)
{
	if (fSendBuffer.Len == 0) {
		for (UInt32 count = 0; count < inNumVecs; count++) {
			...
			Assert(fSendBuffer.Len < CDMSysSocket::kSendBufferLen);
		}
	}
...
OS_Error CTCPDMSysSocket::Read(void *inBuffer, const UInt32 inLength, UInt32 *outRcvLen)
{
...
////////////////////CHTTPDMSysSocket 实现////////////////////
CHTTPDMSysSocket::CHTTPDMSysSocket(const StrPtrLen &inURL, UInt32 inCookie, UInt32 inSocketType)
	: fCookie(inCookie),
...
CHTTPDMSysSocket::~CHTTPDMSysSocket()
{
	delete[] fURL.Ptr;
	delete fPostSocket;
}

OS_Error CHTTPDMSysSocket::Read(void *inBuffer, const UInt32 inLength, UInt32 *outRcvLen)
{
	//
	// Bring up the GET connection if we need to
	if (!fGetSocket.IsConnected()) {
#if CLIENT_SOCKET_DEBUG
		qtss_printf("CHTTPDMSysSocket::Read: Sending GET\n");
#endif
...
#if CLIENT_SOCKET_DEBUG
				qtss_printf("CHTTPDMSysSocket::Read: Received GET response\n");
#endif
...
#if CLIENT_SOCKET_DEBUG
		qtss_printf("CHTTPDMSysSocket::Read: Waiting for GET response\n");
#endif
	...
	if (theErr != OS_NoErr) {
#if CLIENT_SOCKET_DEBUG
		// qtss_printf("CHTTPDMSysSocket::Read: Waiting for data\n");
#endif
		fSocketP = &fGetSocket;
		fEventMask = EV_RE;
	}
...
#if CLIENT_SOCKET_DEBUG
	// else
	// qtss_printf("CHTTPDMSysSocket::Read: Got some data\n");
#endif
	return theErr;
}

OS_Error CHTTPDMSysSocket::SendV(iovec *inVec, UInt32 inNumVecs)
{
	//
...
#if CLIENT_SOCKET_DEBUG
		qtss_printf("CHTTPDMSysSocket::Send: Sending POST\n");
#endif
...
#if CLIENT_SOCKET_DEBUG
	// qtss_printf("CHTTPDMSysSocket::Send: Sending data\n");
#endif
	return this->SendSendBuffer(fPostSocket);
}

void CHTTPDMSysSocket::EncodeVec(iovec *inVec, UInt32 inNumVecs)
{
	for (UInt32 count = 0; count < inNumVecs; count++) {
...
void CDMSysSource::InitDMSysClient(UInt32 inSocketType)
{
	fLocalAddr = ntohl(inet_addr("192.168.11.222"));
	fDMSysTCPSocket = NEW CTCPDMSysSocket(inSocketType);

	fDMSysClient = NEW CDMSysClient(fDMSysTCPSocket);
	fDMSysClient->InitDMSysClient();

	fHostAddr = fDMSysClient.GetDMSysIP();
	fHostPort = fDMSysClient.GetDMSysPort();
}

void CDMSysSource::SetDMSysClientInfo(UInt32 inAddr, UInt16 inPort, char *inURL, UInt32 inLocalAddr)
{
	if (fDMSysTCPSocket != NULL) {
		fDMSysTCPSocket->Set(inAddr, inPort);
	}

	if (inLocalAddr != 0) {
		fDMSysTCPSocket->GetSocket()->Bind(inLocalAddr, 0);
	}

	int     sndBufSize = 32 * 1024;
	int     rcvBufSize = 1024;
	fDMSysTCPSocket->SetOptions(sndBufSize, rcvBufSize);
}
