...
public:
	...
	void SetFillSize(UInt32 fillSize)
	{
		fBufferFillSize = fillSize;
	}

	UInt32 GetFillSize(void)
	{
		return fBufferFillSize;
	}

	OSQueueElem *GetQElem()
	{
		return &fQElem;
	}
...
public:
	...
	void IncMaxBuffers(void)
	{
		fMaxBuffers += fBufferInc;
	}

	void DecMaxBuffers(void)
	{
	...
	void DecCurBuffers(void)
	{
		if (fNumCurrentBuffers > 0) {
			fNumCurrentBuffers--;
		}
	}
	...
	UInt32 GetNumCurrentBuffers(void)
	{
...
public:
	...
	void TestBuffer(SInt32 bufIndex)
	{
		Assert(bufIndex >= 0); fFileMapArray[bufIndex]->TestBuffer();
	}

	void SetIndexBuffFillSize(SInt32 bufIndex, UInt32 fillSize)
	{
	...
	UInt32 GetBuffSize(SInt64 bufIndex)
	{
		Assert(bufIndex >= 0); return fFileMapArray[bufIndex]->GetFillSize();
	}

	UInt32 GetIncBuffers(void)
	{
	...
	void IncMaxBuffers()
	{
		fBlockPool.IncMaxBuffers();
	}

	void DecMaxBuffers()
	{
		fBlockPool.DecMaxBuffers();
	}
	...
	SInt64 GetBuffIndex(UInt64 inPosition)
	{
		return inPosition / this->GetMaxBufSize();
	}

	SInt64 GetMaxBuffIndex()
	{
		Assert(fMapArraySize > 0); return fMapArraySize - 1;
	}

	UInt64 GetBuffOffset(SInt64 bufIndex)
	{
		return (UInt64)(bufIndex * this->GetMaxBufSize());
	}
...
public:
	...
	void EnableFileCache(Bool16 enabled)
	{
		OSMutexLocker locker( &fMutex); fCacheEnabled = enabled;
	}

	Bool16 GetCacheEnabled()
	{
	...
	void IncMaxBuffers()
	{
		OSMutexLocker locker( &fMutex); fFileMap.IncMaxBuffers();
	}

	void DecMaxBuffers()
	{
		OSMutexLocker locker( &fMutex); fFileMap.DecMaxBuffers();
	}
