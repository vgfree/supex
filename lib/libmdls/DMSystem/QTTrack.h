...
public:
	...
	//
	// Accessors.
	inline Bool16 IsInitialized(void)
	{
		return fIsInitialized;
	}

	inline const char *GetTrackName(void)
	{
		return fTrackName ? fTrackName : "";
	}

	inline UInt32 GetTrackID(void)
	{
		return fTrackHeaderAtom->GetTrackID();
	}

	inline UInt64 GetCreationTime(void)
	{
		return fTrackHeaderAtom->GetCreationTime();
	}

	inline UInt64 GetModificationTime(void)
	{
		return fTrackHeaderAtom->GetModificationTime();
	}

	inline SInt64 GetDuration(void)
	{
		return (SInt64)fTrackHeaderAtom->GetDuration();
	}

	inline Float64 GetTimeScale(void)
	{
		return fMediaHeaderAtom->GetTimeScale();
	}

	inline Float64 GetTimeScaleRecip(void)
	{
		return fMediaHeaderAtom->GetTimeScaleRecip();
	}

	inline Float64 GetDurationInSeconds(void)
	{
	...
	inline UInt64 GetFirstEditMovieTime(void)
	{
		if (fEditListAtom != NULL) {
			return fEditListAtom->FirstEditMovieTime();
		} else {                                              return 0; }
	}

	inline UInt32 GetFirstEditMediaTime(void)
	{
		return fFirstEditMediaTime;
	}
	...
	inline void GetPreviousSyncSample(UInt32 SampleNumber, UInt32 *SyncSampleNumber)
	{
		if (fSyncSampleAtom != NULL) {
			fSyncSampleAtom->PreviousSyncSample(SampleNumber, SyncSampleNumber);
	...
	{
		if (fSyncSampleAtom != NULL) {
			fSyncSampleAtom->NextSyncSample(SampleNumber, SyncSampleNumber);
	...
	{
		if (fSyncSampleAtom != NULL) {
			return fSyncSampleAtom->IsSyncSample(SampleNumber, SyncSampleCursor);
		} else {                          return true; }
	}
	...
	inline void DumpSampleToChunkTable(void)
	{
		fSampleToChunkAtom->DumpTable();
	}

	inline void DumpChunkOffsetTable(void)
	{
		fChunkOffsetAtom->DumpTable();
	}

	inline void DumpSampleSizeTable(void)
	{
		fSampleSizeAtom->DumpTable();
	}

	inline void DumpTimeToSampleTable(void)
	{
		fTimeToSampleAtom->DumpTable();
	}

	inline void DumpCompTimeToSampleTable(void)
	{
		if (fCompTimeToSampleAtom) {
			fCompTimeToSampleAtom->DumpTable();
		} else {                                                                                                                       printf("*** no ctts table ****\n"); }
	}
