...
void OSHeap::Insert(OSHeapElem *inElem)
{
	...
	if ((fHeap == NULL) || (fFreeIndex == fArraySize)) {	// Heap数组为空或数组存储已满，2倍扩充
		fArraySize *= 2;
...
OSHeapElem *OSHeap::Remove(OSHeapElem *elem)
{
	...
	for (; theIndex < fFreeIndex; theIndex++) {
		if (elem == fHeap[theIndex]) {	// 找到将要移除的Elem
			break;
		}
	}
