...
private:
	...
	inline void AdjustState(int i)
	{
		fState += i;
	}

	inline void AdjustWriteWaiters(int i)
	{
		fWriteWaiters += i;
	}

	inline void AdjustReadWaiters(int i)
	{
		fReadWaiters += i;
	}
	...
	inline void SetWriteWaiters(int i)
	{
		fWriteWaiters = i;
	}

	inline void SetReadWaiters(int i)
	{
		fReadWaiters = i;
	}

	inline void AddWriteWaiter()
	{
		AdjustWriteWaiters(1);
	}

	inline void RemoveWriteWaiter()
	{
		AdjustWriteWaiters(-1);
	}

	inline void AddReadWaiter()
	{
		AdjustReadWaiters(1);
	}

	inline void RemoveReadWaiter()
	{
		AdjustReadWaiters(-1);
	}

	inline void AddActiveReader()
	{
		AdjustState(1);
	}

	inline void RemoveActiveReader()
	{
		AdjustState(-1);
	}

	inline Bool16 WaitingWriters()
	{
		return (Bool16)(fWriteWaiters > 0);
	}

	inline Bool16 WaitingReaders()
	{
		return (Bool16)(fReadWaiters > 0);
	}

	inline Bool16 Active()
	{
		return (Bool16)(fState != 0);
	}

	inline Bool16 ActiveReaders()
	{
		return (Bool16)(fState > 0);
	}

	inline Bool16 ActiveWriter()
	{
		return (Bool16)(fState < 0);
	}								// only one
...
public:
	...
	void SetMutex(OSMutexRW *mutexPtr)
	{
		fRWMutexPtr = mutexPtr;
	}
...
public:
	...
	{
		if (OSMutexReadWriteLocker::fRWMutexPtr != NULL) {
