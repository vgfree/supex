...
#elif __PTHREADS__
	pthread_key_create(&OSThread::gMainKey, NULL);	// Thread key
  #ifdef _POSIX_THREAD_PRIORITY_SCHEDULING
	//
	// Added for Solaris...
	pthread_attr_init(&sThreadAttr);// Thread Attribute
	// qtss_printf("Thread Attributes:\n\t[detach]:%d\n\t[schedule]:%d\n\t[inherit]:%d\n\t[scope]:%d\n\t[stacksize]:%d\n"
	//            );
...
void OSThread::Start()
{
...
#elif __PTHREADS__
	// qtss_printf("*********** OSThread::Start __PTHREADS__ ***********\n");
	pthread_attr_t *theAttrP;
  #ifdef _POSIX_THREAD_PRIORITY_SCHEDULING
	// qtss_printf("*********** OSThread::Start _POSIX_THREAD_PRIORITY_SCHEDULING **********\n");
	...
	if (0 != err) {
		qtss_printf("*********** OSThread::Start create thread error ***********\n");
	}
...
#endif
{
	...
	// qtss_printf("********** OSThread::_Entry [__PTHREADS__] thread <%ld, %ld>**********\n", theThread->fThreadID, syscall(__NR_gettid));
	pthread_setspecific(OSThread::gMainKey, theThread);	// 线程私有数据
#else
	theThread->fThreadID = (UInt32)cthread_self();
	cthread_set_data(cthread_self(), (any_t)theThread);
	// qtss_printf("********** OSThread::_Entry thread %ld**********\n", theThread->fThreadID);
#endif
