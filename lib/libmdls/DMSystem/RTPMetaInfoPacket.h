...
public:
	...
	enum
	{
		kPacketPosField = 0,		// TW0_CHARS_TO_INT('p', 'p'),位置
		kTransTimeField = 1,		// TW0_CHARS_TO_INT('t', 't'),传输时间
		kFrameTypeField = 2,		// TW0_CHARS_TO_INT('f', 't'),帧类型
		kPacketNumField = 3,		// TW0_CHARS_TO_INT('p', 'n'),包序号？包數
		kSeqNumField = 4,		// TW0_CHARS_TO_INT('s', 'q'),序号
		kMediaDataField = 5,		// TW0_CHARS_TO_INT('m', 'd'),媒体类型
	...
	typedef UInt16 FieldName;	// unsigned short
	typedef SInt32 FieldID;		// signed long
	...
	typedef UInt16 FrameTypeField;	// 帧类型
	...
	//
	// Field Accessors
	SInt64 GetTransmitTime()
	{
		return fTransmitTime;
	}

	FrameTypeField GetFrameType()
	{
		return fFrameType;
	}

	UInt64 GetPacketNumber()
	{
		return fPacketNumber;
	}

	UInt64 GetPacketPosition()
	{
		return fPacketPosition;
	}
	...
	UInt32 GetMediaDataLen()
	{
		return fMediaDataLen;
	}
