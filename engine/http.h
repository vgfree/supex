...
struct http_parse_info
{
	...
	struct http_status      hs;	/**< HTTP数据分析器状态*/
};
 ...
 */
bool http_parse_response(struct http_parse_info *info);

/**
 * 解析请求http字符串
 * @return false 还未结束，需要后续数据；ture 已经结束，根据内部状态判断成功与否
 */
bool http_parse_request(struct http_parse_info *info);
 ...
 */
struct net_cache;
void http_response(struct net_cache *cache, int code, int pos);
