...
local OWN_REDIS_INDEX = {}

local function new_connect(redname)
	local nb = 0
	OWN_REDIS_INDEX[redname] = (OWN_REDIS_INDEX[redname] or 0)
	repeat
		nb = nb + 1
		ok,OWN_REDIS_POOLS[redname] = pcall(redis.connect, APP_LINK_REDIS_LIST[redname]["host"], APP_LINK_REDIS_LIST[redname]["port"])
		if not ok then
			OWN_REDIS_INDEX[redname] = OWN_REDIS_INDEX[redname] + 1
			socket.select(nil, nil, 0.05 * ((OWN_REDIS_INDEX[redname] >= MAX_DELAY) and MAX_DELAY or OWN_REDIS_INDEX[redname]))
			only.log("E", OWN_REDIS_POOLS[redname])
			only.log("I", string.format('REDIS_NAME: %s | INDEX: %d |---> Tcp:connect: FAILED!', redname, OWN_REDIS_INDEX[redname]))
	...
	only.log("I", string.format('REDIS_NAME: %s | INDEX: %d |---> Tcp:connect: SUCCESS!', redname, OWN_REDIS_INDEX[redname]))
	OWN_REDIS_INDEX[redname] = 0
	return true
end
