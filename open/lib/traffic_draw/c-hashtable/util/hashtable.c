...
#define DECONNECT_FROM_BUCKET_DLLIST(element, list_head)	    \
	do {							    \
		if ((element)->pLast) {				    \
...
#define DECONNECT_FROM_GLOBAL_DLLIST(element, ht)				\
	do {									\
...
HashTable *_create_hashtable(uint size, const char *s_typename)
{
	...
	if (strcmp(trim(str_array[1], 3), "int") &&
		...
		strcmp(trim(str_array[1], 3), "intptr_t") &&
		strcmp(trim(str_array[1], 3), "char")) {
		free(str_array[0]);
		free(str_array);
		return NULL;
...
int _hash_add(HashTable *ht, ...)
{
	...
	} else if (strcmp(ht->valueType, "intptr_t") == 0) {
		(*(intptr_t *)value) = va_arg(vlist, intptr_t);
